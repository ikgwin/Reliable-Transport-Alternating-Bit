...
int ComputeChecksum(struct pkt packet)
{
  ...
  int i ;
  // add ack n seq nums together
  checksum += (int)packet.acknum+(int)packet.seqnum;

  // add all the bytes up that are in thee payload
  for (int i=0; i<20; i++) {
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum != old_ack) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      ...
      /***** 1. FILL IN CODE  What else needs to be done when an ACK arrives
       ...
       stoptimer(A); // stop timer when ack receive
    }
    else
      if (TRACE > 0)
...
void A_timerinterrupt(void)
{
  ...
  // we resend packet so we restart timer
  starttimer(A, RTT);
  packets_resent++;
}       
